...
export default function CheckoutPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!name || !email || !phone || !address || !city || !country) {
      setError("Please complete all shipping information before continuing.");
      return;
    }
    ...
    try {
      const response = await fetch("/api/stripe/create-checkout-session", {
        ...
        body: JSON.stringify({
          items,
          customerEmail: email,
          shipping: { name, phone, address, city, country },
        }),
      });
      ...
      if (!response.ok || !data.url) {
        throw new Error(
          data.error ||
            "Checkout details were saved, but payment is not configured yet. We will connect the payment method last."
        );
      }
      ...
      <div className="container checkout-wrap">
        <div className="checkout-heading">
          ...
          <p className="checkout-note">
            Review your order and enter your shipping information. Payment configuration will be connected at the final stage.
          </p>
        </div>
        ...
        <div className="checkout-layout">
          <form className="checkout-card" onSubmit={handleSubmit} noValidate>
            ...
            <button className="button dark checkout-button" type="submit" disabled={loading}>
          ...
          <aside className="checkout-summary">
            <div className="checkout-summary-header"><h2>Order summary</h2><span>{items.length} {items.length === 1 ? "item" : "items"}</span></div>
            <div className="checkout-items">
              {items.map((item) => (
                <div className="checkout-item" key={item.productId}>
                  <div className="checkout-item-thumb">
                    {item.imageUrl ? <img src={item.imageUrl} alt="" /> : <span>{item.name.charAt(0)}</span>}
                  </div>
