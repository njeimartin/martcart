...
export default async function HomePage(){
  ...
  return <main className="triggers-site">
    ...
    <header className="trigger-nav"><div className="trigger-container nav-inner">
      <Link className="trigger-logo" href="/"><span className="logo-mark">TN</span><span>TRIGGERS<br/><b>NATION</b></span></Link>
      <nav><Link href="/">HOME</Link><Link href="/shop">SHOP</Link><Link href="/about">ABOUT US</Link><Link href="/blog">BLOG</Link><Link href="/contact">CONTACT</Link></nav>
      <div className="trigger-actions"><Link href="/shop">SEARCH</Link><Link href="/auth">ACCOUNT</Link><Link href="/cart">CART</Link></div>
    </div></header>
    <section className="trigger-hero"><div className="trigger-container hero-overlay"><p className="trigger-kicker">GEAR UP. TRAIN HARD. STAY READY.</p><h1>BUILT FOR<br/><em>PERFORMANCE.</em></h1><p className="hero-tagline">READY FOR ANYTHING.</p><div className="trigger-buttons"><Link className="trigger-btn gold" href="/shop">SHOP NOW →</Link><a className="trigger-btn outline" href="#categories">EXPLORE COLLECTION</a></div></div></section>
    ...
    <section className="instagram-section dark-section"><div className="trigger-container trigger-section"><div className="trigger-heading"><div><span>@TRIGGERSNATION</span><h2>FOLLOW THE MISSION</h2></div><Link href="/contact">FOLLOW US →</Link></div><div className="instagram-grid">{instagramImages.map((src,i)=><a href="/blog" aria-label={`Triggers Nation Instagram post ${i+1}`} key={`${src}-${i}`}><img src={src} alt="Triggers Nation tactical gear and outdoor scene" loading="lazy"/></a>)}</div></div></section>
